...
test('scanForIdentifiers catches what a leak actually looks like', () => {
  assert.ok(nf.scanForIdentifiers('mail from Selvi Doruca', []).length, 'two capitalised words');
...
test('a note carrying real data cannot be rendered at all', () => {
  assert.throws(
    () => nf.renderFeedbackNote({ ...GOOD, symptom: 'Selvi Doruca got the wrong file.' }, []),
    /redact|identifier/i);
  assert.throws(
...
test('scanForIdentifiers catches a name written in ALL CAPS', () => {
  assert.ok(nf.scanForIdentifiers('mail from SELVI DORUCA about the file', []).length,
...
test('scanForIdentifiers catches a bare @handle', () => {
  assert.ok(nf.scanForIdentifiers('ping @selvidoruca about this', []).length, 'a handle, not an email');
});
...
test('scanForIdentifiers still catches a name split by exactly one hard line-wrap, but not by a blank line', () => {
  ...
  assert.ok(nf.scanForIdentifiers('...written by Selvi\nDoruca during review.', []).length,
    'a name split by an ordinary hard line-wrap');
  assert.deepStrictEqual(nf.scanForIdentifiers('Selvi\n\nDoruca', []), [],
...
test('the feedback skill names no one and ships no real example', () => {
  const text = fs.readFileSync(path.join(PLUGIN_ROOT, 'skills', 'feedback', 'SKILL.md'), 'utf8');
  assert.doesNotMatch(text, /Selvi|Serkan|Doruca|Vektura/i, 'no real people or workspaces');
});
...
test('scanForIdentifiers sees through a Turkish suffix on a forbidden word', () => {
  ...
  assert.ok(nf.scanForIdentifiers('Doruca\u2019nın notu geldi', ['Doruca']).length, 'curly-apostrophe suffix');
  assert.ok(nf.scanForIdentifiers("Doruca'nın notu geldi", ['Doruca']).length, 'straight-apostrophe suffix');
});
...
test('scanForIdentifiers still catches every rooted path shape after the token-start anchor', () => {
  for (const t of [
    'It wrote to D:\\work\\clients\\selvi-doruca\\mail.md',
