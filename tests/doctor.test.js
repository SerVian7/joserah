...
test('a project that is itself a repo directly under projects/ is audited as itself, not via its internals', (t) => {
  ...
  const projDir = path.join(dir, 'projects', 'Vektura-SDWAN');
  ...
  const r = runTool('doctor.js', [dir]);
  assert.strictEqual(r.status, 0, r.stdout);
  assert.match(r.stdout, /^warn {2}.*projects\/Vektura-SDWAN\s+—.*no remote/m,
    'the project itself is audited and reported for having no remote');
  assert.ok(!/projects\/Vektura-SDWAN\/\.git/.test(r.stdout), 'never audits the repo\'s own .git directory');
  assert.ok(!/projects\/Vektura-SDWAN\/docs/.test(r.stdout), 'never audits the repo\'s own subfolders as separate projects');
});
...
// Until 0.13.0 this place held `AGENTS.md + MAX_LAYER_CHARS > WARN_TOTAL_CHARS`,
// on the premise that the total was the thing being truncated, so the warning
// had to arrive before the total could reach a cuttable size. That premise is
// retired on purpose: the cut is now per file (MAX_LAYER_CHARS) and per hook
// command (MAX_HOOK_CHARS), and neither is a function of the total, so the old
// assertion no longer expresses anything true. What replaces it is the
// guarantee that does hold — a layer over its cap is cut, and the injected
// text says so and names the file, so the owner hears it from the briefing
// rather than from a number that no longer tracks it. WARN_TOTAL_CHARS is now
// about what a session costs, not about what it loses.
test('a standing layer over its cap is cut, and the text says so', (t) => {
  const { MAX_LAYER_CHARS, MAX_HOOK_CHARS, directivesBlock } =
    require(path.join(PLUGIN_ROOT, 'hooks', 'lib', 'standing-context'));
  assert.ok(MAX_LAYER_CHARS < MAX_HOOK_CHARS,
    'no single layer may fill a whole hook command and starve the others');

  const dir = freshWs(t);
  fs.writeFileSync(path.join(dir, '.joserah', 'directives.md'),
    '# Directives — w\n\n## Scope\n\n' + '- a standing rule that is here to take up room.\n'.repeat(400));
  const block = directivesBlock(dir);
  assert.match(block, /\[cut\] \.joserah\/directives\.md is longer than \d+ characters/,
    'a cut layer has to announce itself inside the text it was cut from');
  assert.ok(block.length < MAX_HOOK_CHARS,
    `one layer came back at ${block.length} characters, which is a whole hook command`);
});

// The case the warning exists for, measured rather than assumed. 22,284
// characters is the real size of one live workspace's directives file. While
// standingContextSize() summed the capped blocks, that workspace was counted at
// the 5,000 cap and reported healthy while its rules were being cut — a warning
// measuring its own truncation, which is worse than no warning because it
// actively reassures.
test('a directives file far over the layer cap is counted at its real size and warns', (t) => {
  const dir = freshWs(t);
  const head = '# Directives — w\n\n## Scope\n\n';
  const line = '- a standing rule of the owner, written out at a realistic length.\n';
  let body = head;
  while (body.length < 22284) body += line;
  fs.writeFileSync(path.join(dir, '.joserah', 'directives.md'), body.slice(0, 22284));

  const r = runTool('doctor.js', [dir]);
  assert.strictEqual(r.status, 0, r.stdout);
  assert.match(r.stdout, /^warn\s+standing context size\s+— \d+ characters/m);
  const total = Number(r.stdout.match(/standing context size\s+— (\d+) characters/)[1]);
  assert.ok(total > 22284,
    `doctor reported ${total} characters for a workspace whose directives file alone is 22,284`);
});
...
// The public repository's history is going to be wiped and force-pushed. Every
// machine's marketplace clone then stops fast-forwarding, the daily
// `git pull --ff-only` fails, and the owner simply stops getting updates with
// nothing saying so. These three tests are the "nothing saying so" part.
//
// The whole fixture is local: a bare repo plays the remote, a real clone of it
// plays the marketplace clone under its own CLAUDE_CONFIG_DIR. Nothing reaches
// the network.
function marketplaceCloneFixture(t) {
  const { spawnSync } = require('child_process');
  const configDir = tmpdir(t);
  const g = (cwd, a) => spawnSync('git', ['-C', cwd, '-c', 'user.email=t@example.invalid',
    '-c', 'user.name=T', ...a], { encoding: 'utf8' });
  const seed = path.join(configDir, 'seed');
  const bare = path.join(configDir, 'origin.git');
  const clone = path.join(configDir, 'plugins', 'marketplaces', 'joserah');
  fs.mkdirSync(seed, { recursive: true });
  g(seed, ['init', '--quiet']);
  fs.writeFileSync(path.join(seed, 'a.txt'), 'one\n');
  g(seed, ['add', '-A']);
  g(seed, ['commit', '--quiet', '-m', 'one']);
  ...
  g(seed, ['remote', 'add', 'origin', bare]);
  g(seed, ['push', '--quiet', 'origin', 'main']);
  fs.mkdirSync(path.dirname(clone), { recursive: true });
  spawnSync('git', ['clone', '--quiet', bare, clone], { encoding: 'utf8' });
  fs.writeFileSync(path.join(configDir, 'plugins', 'known_marketplaces.json'),
    JSON.stringify({ joserah: { source: { source: 'git', url: bare }, installLocation: clone } }, null, 2));
  return { configDir, clone, seed, g };
}

test('a force-pushed marketplace remote is reported, not silently stale', (t) => {
  const { spawnSync } = require('child_process');
  if (spawnSync('git', ['--version']).status !== 0) return t.skip('git unavailable');
  const dir = freshWs(t);
  const fx = marketplaceCloneFixture(t);
  // Exactly what wiping the public repo's history and force-pushing does: the
  // branch now points at an unrelated root commit.
  fx.g(fx.seed, ['checkout', '--quiet', '--orphan', 'fresh']);
  fs.writeFileSync(path.join(fx.seed, 'b.txt'), 'unrelated\n');
  fx.g(fx.seed, ['add', '-A']);
  fx.g(fx.seed, ['commit', '--quiet', '-m', 'unrelated root']);
  fx.g(fx.seed, ['push', '--quiet', '--force', 'origin', 'fresh:main']);

  const r = runTool('doctor.js', [dir], { env: { CLAUDE_CONFIG_DIR: fx.configDir } });
  assert.match(r.stdout, /^warn\s+marketplace clone diverged from its remote\s+—/m, r.stdout);
  assert.match(r.stdout, /reset --hard/, r.stdout);
});

test('an untouched marketplace clone says nothing at all', (t) => {
  const { spawnSync } = require('child_process');
  if (spawnSync('git', ['--version']).status !== 0) return t.skip('git unavailable');
  const dir = freshWs(t);
  const fx = marketplaceCloneFixture(t);
...
test('no marketplace clone at all says nothing at all', (t) => {
  const dir = freshWs(t);
  const configDir = tmpdir(t);
  fs.mkdirSync(path.join(configDir, 'plugins'), { recursive: true });
  fs.writeFileSync(path.join(configDir, 'plugins', 'known_marketplaces.json'),
    JSON.stringify({ joserah: { installLocation: path.join(configDir, 'gone') } }, null, 2));
  const r = runTool('doctor.js', [dir], { env: { CLAUDE_CONFIG_DIR: configDir } });
  assert.ok(!/marketplace clone diverged/.test(r.stdout), r.stdout);
  assert.strictEqual(r.status, 0, r.stdout + r.stderr);
});
