...
test('scan stops at a nested workspace and reports it as a boundary', (t) => {
  ...
  write(dir, 'guestws/.joserah/knowledge/people/selvi.md', '# Selvi\n');
...
test('migrate appends a Relations block for entities mentioned in prose', (t) => {
  const dir = ws(t);
  write(dir, '.joserah/knowledge/wiki/entities/meridyen.md', '# Meridyen\n\nA client.\n');
  const journal = '# 2026-08-30\n\nFixed the Meridyen switch fabric today.\n';
  ...
  assert.match(text, /- mentions \[\[Meridyen\]\]/);
});
...
test('migrate does not duplicate a relation that already exists', (t) => {
  ...
  runTool('migrate.js', [dir]);
  const text = fs.readFileSync(path.join(dir, '.joserah/desk/daily/2026/2026-08-30.md'), 'utf8');
  assert.strictEqual((text.match(/\[\[Meridyen\]\]/g) || []).length, 1);
});

test('migrate does not relate an entity note to itself', (t) => {
  const dir = ws(t);
  const rel = '.joserah/knowledge/wiki/entities/meridyen.md';
  write(dir, rel, '# Meridyen\n\nMeridyen is a client.\n');
  runTool('migrate.js', [dir]);
  const text = fs.readFileSync(path.join(dir, rel), 'utf8');
  assert.doesNotMatch(text, /- mentions \[\[Meridyen\]\]/);
});

test('R3: a wikilink-shaped mention inside a code fence produces no relation', (t) => {
  const dir = ws(t);
  write(dir, '.joserah/knowledge/wiki/entities/meridyen.md', '# Meridyen\n\nA client.\n');
  const journal = '# 2026-08-30\n\n```\n[[Meridyen]] switch fabric config\n```\n\nUnrelated notes today.\n';
  write(dir, '.joserah/desk/daily/2026/2026-08-30.md', journal);
  runTool('migrate.js', [dir]);
  const text = fs.readFileSync(path.join(dir, '.joserah/desk/daily/2026/2026-08-30.md'), 'utf8');
...
test('CRITICAL 1: a feedback note is out of the migration scan entirely', (t) => {
  const dir = ws(t);
  write(dir, '.joserah/feedback/prompt/2026-08-30-selvi-doruca-mail-loop.md',
    '---\ntype: feedback\narea: prompt\n---\n\n## Symptom\n\nSomething.\n');
  const { files } = scanWorkspace(dir);
...
test('CRITICAL 1: a rendered feedback note is byte-identical after a migration run', (t) => {
  ...
  const rel = '.joserah/feedback/prompt/2026-08-30-selvi-doruca-mail-loop.md';
  write(dir, rel, note);
  const r = runTool('migrate.js', [dir]);
  assert.strictEqual(r.status, 0, r.stderr);
...
// Three plugin files live inside the workspace and are byte-compared by
// doctor. scaffold.js writes them and refuses to run twice, so before 0.13.0
// an existing workspace could only get them by someone following a list — and
// the list named two of the three. migrate.js is the tool that exists to carry
// a workspace forward; it carries these too.
test('migrate installs and refreshes the workspace copies of the plugin tools', (t) => {
  const dir = freshWs(t);
  const local = (...p) => path.join(dir, '.joserah', 'tools', ...p);
  const plugin = (...p) => path.join(PLUGIN_ROOT, 'tools', ...p);

  fs.rmSync(local('secret.js'));
  fs.writeFileSync(local('verify-links.js'), '// a stale copy from an older plugin\n');

  const r = runTool('migrate.js', [dir]);
  assert.strictEqual(r.status, 0, r.stdout + r.stderr);
  for (const rel of [['secret.js'], ['verify-links.js'], ['lib', 'untouchable.js']]) {
    assert.strictEqual(fs.readFileSync(local(...rel), 'utf8'), fs.readFileSync(plugin(...rel), 'utf8'),
      `.joserah/tools/${rel.join('/')} does not match the plugin copy`);
  }

  // Doctor is the thing that was failing, so it is the real assertion.
  const d = runTool('doctor.js', [dir]);
  assert.doesNotMatch(d.stdout, /fail\s+local secret\.js current/);
  assert.doesNotMatch(d.stdout, /fail\s+local verify-links\.js current/);
});

test('migrate leaves an already-current tool copy alone and says nothing about it', (t) => {
  const dir = freshWs(t);
  const r = runTool('migrate.js', [dir]);
  assert.doesNotMatch(r.stdout, /secret\.js/, 'a workspace already current has nothing to report');
});

test('migrate --dry-run decides the same way but writes no tool copy', (t) => {
  const dir = freshWs(t);
  fs.rmSync(path.join(dir, '.joserah', 'tools', 'secret.js'));
  runTool('migrate.js', [dir, '--dry-run']);
