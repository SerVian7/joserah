...
test('secret-scan does not report a clean tree when a listed file cannot be read', (t) => {
  // This used to delete ghost.md and leave it at that — but a file gone
  // from disk (ENOENT) is exactly the case 0.13.0 reclassified as `missing`,
  // not a scanning failure (see the "deleted from disk" test below). To
  // still exercise the genuinely-unreadable branch, a directory is put back
  // where git expects a file: EISDIR, not ENOENT — the file IS there and
  // cannot be read, which must still exit 2.
  const d = path.join(tmpdir(t), 'ws');
  fs.mkdirSync(d, { recursive: true });
  spawnSync('git', ['init', '-q'], { cwd: d });
  ...
  fs.unlinkSync(ghost); // still tracked in the git index
  fs.mkdirSync(ghost);  // ...but a directory sits where the file was listed
  const r = runTool('secret-scan.js', [d]);
...
// A file deleted from the working tree but not yet staged is still in
// `git ls-files`. Until 0.13.0 that made the scan exit 2 — "treating the
// workspace as unscanned" — which is the gate in front of every repository
// backup. Deleting a note mid-sweep is not corruption.
test('a tracked file deleted from disk does not make the scan report the workspace unscanned', (t) => {
  ...
  git('add', '-A');
  git('commit', '-qm', 'first');
  fs.unlinkSync(path.join(dir, 'note.md'));   // deleted, deliberately NOT staged

  const r = runTool('secret-scan.js', [dir]);
  assert.strictEqual(r.status, 0, `exit ${r.status}: ${r.stdout}${r.stderr}`);
  assert.match(r.stdout, /No credential-shaped content found/);
