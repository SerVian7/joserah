...
// Doctor talks to the network in exactly one place (the marketplace-clone
...
const CHECKS = [
...

  {
    ...
    remedies: [
      {
        key: '`local secret.js current` FAIL',
        text: 'Copy the plugin\'s `tools/secret.js` over `.joserah/tools/secret.js` — the vault tool the prompt calls by that path. It creates `keys/secrets.json` on its first `--set`; never read that file. Re-run doctor.',
      },
    ],
    // 0.12.0: the vault tool travels the same way as verify-links.js — copied
    // at scaffold time, compared here, re-copied by update on a FAIL.
    run({ root, pluginDir, normalizeEol }) {
...

  {
    ...
    // nothing else in this tool would ever say so — the owner just stops
    // getting updates. What this CANNOT do offline: after a force-push the
    // clone's own origin/* refs are stale too, so the divergence is invisible
    // until something fetches. So this fetches — with a short timeout, and
    // failing open in every direction: no git, no clone, no upstream, no
    // network, a timeout or any other error prints nothing at all. Doctor must
    // never fail, hang, or invent a finding because the network was down.
    // The clone is the one Claude Code recorded (prompt.js reads
    // known_marketplaces.json under CLAUDE_CONFIG_DIR) — not a guessed path.
    run() {
      const { marketplaceCloneDir } = require('./prompt');
      const { sameRepoPath } = require('./git-root');
      const clone = marketplaceCloneDir();
      if (!clone || !fs.existsSync(clone)) return [];
      // Same P0-1 guard as project-backups: `git -C <dir>` answers for the
      // nearest ANCESTOR repo when <dir> is not one itself.
      const top = gitIn(clone, ['rev-parse', '--show-toplevel']);
      if (top === null || !sameRepoPath(top, clone)) return [];
      const git = (argv) => spawnSync('git', ['-C', clone, ...argv], { encoding: 'utf8', timeout: FETCH_TIMEOUT_MS });
      const upstream = git(['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{u}']);
      if (upstream.status !== 0) return [];
      const ref = upstream.stdout.trim();
      if (git(['fetch', '--quiet']).status !== 0) return [];
      // Exactly the question `pull --ff-only` asks. Exit 1 means "not an
      // ancestor" — diverged; 0 means fine; anything else is an error and is
      // not a finding.
      if (git(['merge-base', '--is-ancestor', 'HEAD', ref]).status !== 1) return [];
      return [warn('marketplace clone diverged from its remote',
