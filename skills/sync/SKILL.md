...
> **Running the plugin's tools.** That command uses `${CLAUDE_PLUGIN_ROOT}`.
> It expands in bash; in PowerShell it is variable syntax, not an environment
> lookup, and expands to nothing — leaving you running `node "/tools/…"`.
